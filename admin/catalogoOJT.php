...
$sqlEspecialidad2 = "SELECT * FROM categorias WHERE estado = 0 ";
$especialidad2 = mysqli_query($conexion,$sqlEspecialidad2);
?>
                ...
                <div class="row" id="listprincojt">
                    <div class="col-xs-12">
                        <div class="box">
                            ...
                            <div class="box-body">
                                <table style="width: 100%;" id="data-table-proOJT"
...
?>
    ...
    ["<?php echo $data['id_pers']?>", "<?php echo $data['gstNombr']?>", "<?php echo $data['gstApell']?>",
        ...
        "<?php echo "<a title='Ver detalle' onclick='opencomiOJT($perona)' type='button' data-toggle='modal' data-target='' class='editar btn btn-default'><i class='fa fa-list-ul'></i></a>"?>"
    ],
...
//FUNCIÓN PARA TRAER EL DETALLE DE LAS COMISIONES
function opencomiOJT(perona) {
    var pe1 = perona
    $("#comision_dett").toggle(250); //Muestra contenedor 
    $("#listaOJT").toggle("fast"); //Oculta lista
    ...
    var id = perona
    var tableCursosProgramados = $('#data-table-OJTprogramcomis').DataTable({
        "order": [
            [3, "asc"]
        ],
        "ajax": {
            "url": "../php/programcomiojt.php",
            "type": "GET",
            "data": function(d) {
                d.id = id;
            }
...
function dettcomis() {
    $("#comision_dett").toggle(250); //Muestra contenedor 
    $("#listprincojt").toggle("fast"); //Oculta lista
    $("#detproOJT").toggle("fast"); //Oculta lista
    $("#data-table-OJTProgramados").dataTable().fnDestroy();
    
   

}
//FUNCIÓN PARA TRAER EL DETALLE DE LA TAREAS Y SUBTAREAS DE EL INSPECTOR
function openOJT(perona) {
    //alert("entra detalles");
    //alert(perona);
    var pe1 = perona
    
    $("#detproOJT").toggle(250); //Muestra contenedor 
    $("#listprincojt").toggle("fast"); //Oculta lista
    $("#comision_dett").toggle("fast"); //Oculta lista
    //alert(perona);
    document.getElementById('inspercooj').value = perona;
    $("#data-table-OJTprogramcomis tr").on('click', function() {
        var comision = "";
        comision += $(this).find('td:eq(1)').html(); //Toma el id de la persona 
        //alert(comision);
        document.getElementById('infcomis1').value = comision;
        $.ajax({
            url: '../php/conPerson.php',
            type: 'POST'
        }).done(function(respuesta) {
            ...
            for (U = 0; U < res.length; U++) {
                if (obj.data[U].gstIdper == perona) {
                    ...
                    $("#nominsp").html('INSPECTOR / ' + d[0] + ' ' + d[1]);
                    $("#nominsp1").html('INSPECTOR / ' + d[0] + ' ' + d[1]);
                    $("#namecomis").html('COMISIÓN -' + comision);
                }
            }
        });
        var id = perona
...
//FUNCION PARA TRAER LA INFORMACIÓN DE LA COMISIÓN EN DETALLES
function comisioninfo(idregisc) {
    //alert(idregisc);
    document.getElementById('idcomiregris').value = idregisc;
    $("#data-table-OJTprogramcomis tr").on('click', function() {
        var comision = "";
        comision += $(this).find('td:eq(1)').html(); //Toma el id de la persona 
        // alert(comision);
    ...
    $.ajax({
        ...
        var x = 0;
        for (U = 0; U < res.length; U++) {
            if (obj.data[U].id_proojt == idregisc) {
                //alert(pruebas);
