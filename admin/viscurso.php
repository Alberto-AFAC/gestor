				...
				<div class="modal-header">
					...
					<div class="modal-body">
						<form id="Dtall" class="form-horizontal" action="" method="POST">
							...
							<div id="agrdiaHabil"></div>
							<div id="edihabilDias"></div>
							<div class="form-group"><br>
								<div class="col-sm-offset-0 col-sm-12">
									<button type="button" id="btnAgrfecha" style="display: none; float: left; font-size:18px; width:150px; height:40px"
									class="btn btn-block btn-success altaboton" onclick="agregarDias();">AGREGAR FECHA</button>

									<button type="button" id="ocubotn" style="float: left; font-size:18px; width:120px; height:40px"
									class="btn btn-block btn-primary altaboton" onclick="validaDias();">VALIDAR
								DÍAS</button>
								<button type="button" id="btnElimfecha" style="color:black; background: silver; float:right; border:1px solid silver; font-size:18px; width:240px; height:40px"
								class="btn btn-block btn-primary altaboton" onclick="eliminarFecha();">ELIMINAR FECHA DE CURSO</button>
							</div>
							<b>
...
<div class="nav-tabs-custom">
	...
	<div class="form-group">
		<div class="col-sm-4">
			<button type='button' title='Días Hábiles' onclick='diasEditar()' class='btn btn-info btnedidh' data-toggle='modal' data-target='#diahabil-modal' id="modalMost" disabled='disabled'>VALIDAR DÍAS HÁBILES </button>

			<a type='button' style="display: none;" title='Días Hábiles' onclick='hrsDias()' class='btn btn-primary btnagrdh' data-toggle='modal' data-target='#diahabil-modal' id="modalMost">AGREGAR DÍAS HÁBILES </a>
		</div>
	</div>
...
<div class="tab-pane" id="timeline">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form id="impri" action="" method="POST">
				...
				<span id="agregarcoordinador" data-toggle="modal" data-target="#agregarInstCord"
				style="font-size:12px; width:280px; height:30px; background: #337ab7;color:white; "
				class="btn btn-sm  altaboton"><i style="color:white;" class="fa fa-user-plus text-info"
				aria-hidden="true"></i> AGREGAR INSTRUCTOR 
			</span>

			<span id="agregarinstructor" data-toggle="modal" data-target="#agregarCordInst"
			style="font-size:12px; width:280px; height:30px; background: #337ab7;color:white; "
			class="btn btn-sm  altaboton"><i style="color:white;" class="fa fa-user-plus text-info"
			aria-hidden="true"></i> AGREGAR COORDINADOR 
		</span>
	...
	<form id="correo" action="" method="POST">
		...
		<div class="modal1">
			...
			<p class="points">Favor de verificar los datos del curso y de los participantes, instructor y coordinador una vez enviado el correo no se podrá realizar ninguna modificación.</p>
...
<div class="modal fade" id='agregarInstCord'>
	<div class="col-xs-12 .col-md-0" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
		<div class="modal-dialog width" role="document" style="/*margin-top: 7em;*/">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span></button>
						<h4 class="modal-title" style="font-size:19px; color: #000000;"></h4>
						<?php

						$sql = "SELECT p.gstIdper,p.gstNombr,p.gstApell,i.cargo, i.* FROM personal p, instruacceso i WHERE p.gstIdper=i.idper AND i.cargo='COORDINADOR' and p.estado = 0 or p.gstIdper=i.idper AND i.cargo='INSTRUCTOR' and p.estado = 0";
					...
					<div class="modal-body">
						<form id="Dtall" class="form-horizontal" action="" method="POST">
							<div class="form-group" id="horario"><br>
								<div class="col-sm-12">
									...
									class="selectpicker" id="idinstinst" name="idinstinst" type="text"
									data-placeholder="SELECCIONE INSTRUCTOR"
									...
									<?php while($instructors = mysqli_fetch_row($instructor)):
										...
										<option value="<?php echo $instructors[0]?>">
											<?php echo $instructors[1].' '.$instructors[2]?>
										</option>
									<?php endwhile; ?>
								</select>
							</div>
						...
						<div class="form-group"><br>
							<div class="col-sm-offset-0 col-sm-12">
								<button type="button" style="float: right; font-size:18px; width:150px; height:40px"
								class="btn btn-block btn-success altaboton" onclick="agregarInstr();">AGREGAR</button>
							</div>
						</div>
					</form>
				</div>
...
<!-- COORDINADOR -->
<div class="modal fade" id='agregarCordInst'>
	<div class="col-xs-12 .col-md-0" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
		<div class="modal-dialog width" role="document" style="/*margin-top: 7em;*/">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						...
						$sql = "SELECT p.gstIdper,p.gstNombr,p.gstApell,i.cargo, i.* FROM personal p, instruacceso i WHERE p.gstIdper=i.idper AND i.cargo in ('COORDINADOR','COORDINADOR_A') and p.estado = 0";
						...
						<form id="Dtall" class="form-horizontal" action="" method="POST">
							<div class="form-group" id="horario"><br>
								<div class="col-sm-12">
									...
									<select style="width: 100%" class="form-control inputalta"
									class="selectpicker" id="idicoordin" name="idicoordin" type="text"
							...
							<div class="form-group"><br>
								<div class="col-sm-offset-0 col-sm-12">
									<button type="button" style="float: right; font-size:18px; width:150px; height:40px"
									class="btn btn-block btn-success altaboton" onclick="agregarCoord();">AGREGAR</button>
								</div>
							</div>
						</form>
					</div>
