...
function curConfir($id_curso,$confir,$justifi,$conexion){
	$query="UPDATE prog_ojt SET confirojt = '$confir', justifojt='$justifi' WHERE id_proojt='$id_curso'";
 ...
 function historia($id,$id_curso,$conexion){
	...
	$query = "INSERT INTO historial(id_usu,proceso,registro,fecha) VALUES ($id,'CONTESTA CONVOCATORIA OJT',concat('RESPUESTA:',' ',(select confirojt from prog_ojt where id_proojt =  $id_curso )),'$fecha')";
	if(mysqli_query($conexion,$query)){
		return true;
	}else{
		return false;
